...
NO_PERM = "`I don't have permission this grp owner is very rude馃槙`"
...
async def promote(promt):
    ...
    try:
        ...
        await promt.edit(f"THIS USER SUCCESSFULLY PROMOTED BY {BOT} 鈿n MY OWNER IS {MASTER} 馃敟")
...
async def demote(dmod):
    ...
    await dmod.edit(f"THIS USER SUCCESSFULLY DEMOTED BY {BOT} 鈿♀殹\n MY MASTER IS {MASTER} 馃敟")
...
async def ban(bon):
    ...
    if reason:
        ...
        await bon.edit(f"BANNED 馃敟`{str(user.id)}` was banned by {BOT} My Master {MASTER}  !!\n\nReason: {reason}")
